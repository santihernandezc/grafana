...
type Suggester = {
  path: NodeTypeName[];
  fun: (node: SyntaxNode, text: string, pos: number) => Suggestion | null;
};

function isPathMatch(suggesterPath: string[], cursorPath: string[]): boolean {
  return suggesterPath.every((item, index) => item === cursorPath[index]);
}
...
const SUGGESTERS: Suggester[] = [
  {
    ...
    fun: suggestLabelKeysWithEquals,
  },
  {
    path: ['PromQL'],
    fun: suggestTopLevel,
  },
  {
    ...
    fun: suggestInFunction,
  },
  {
    path: [ERROR_NODE_NAME, 'LabelMatcher'],
    fun: suggestLabelMatcherError,
  },
  {
    path: ['GroupingLabels'],
    fun: suggestLabelsForGrouping,
  },
];
...
function suggestLabelsForGrouping(node: SyntaxNode, text: string, pos: number): Suggestion | null {
...
function suggestLabelMatcherError(node: SyntaxNode, text: string, pos: number): Suggestion | null {
...
function suggestTopLevel(node: SyntaxNode, text: string, pos: number): Suggestion {
  return {
    type: 'ALL_METRIC_AND_FUNCTION_NAMES',
  };
}

function suggestInFunction(node: SyntaxNode, text: string, pos: number): Suggestion {
  return {
    type: 'ALL_METRIC_NAMES',
  };
}

function suggestLabelKeysWithEquals(node: SyntaxNode, text: string, pos: number): Suggestion | null {
...
export function getSuggestion(text: string, pos: number): Suggestion | null {
...
*/
  ...
  for (let suggester of SUGGESTERS) {
    ...
    // as i find something
    if (isPathMatch(suggester.path, names)) {
      return suggester.fun(currentNode, text, pos);
    }
  }
